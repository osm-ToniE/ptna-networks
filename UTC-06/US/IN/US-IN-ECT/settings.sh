...
OVERPASS_QUERY="http://overpass-api.de/api/interpreter?data=[timeout:900];area[wikidata~'^(Q599694|Q591052|Q856860|Q932018)$'];(rel(area)[~'route'~'(train)'];rel(br);rel[type=%27route%27](r);)-%3E.routes;(.routes;%3C%3C;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
...
PTNA_WWW_REGION_LINK="http://overpass-turbo.eu/map.html?Q=%0A[out%3Ajson][timeout%3A25]%3B%0A%0A(%0A%0A%20%20relation[wikidata~%27^(Q599694|Q591052|Q856860|Q932018)$%27]%3B%0A)%3B%0Aout%20body%3B%0A%3E%3B%0Aout%20skel%20qt%3B"
