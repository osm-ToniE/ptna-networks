...
OVERPASS_QUERY="https://overpass-api.de/api/interpreter?data=area[wikidata='Q785'];(rel(area)[~'route'~'(bus|tram|train|subway|light_rail|trolleybus|ferry|monorail|aerialway|share_taxi|funicular)'];rel(br);rel[type=%27route%27](r);)-%3E.routes;(.routes;%3C%3C;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
NETWORK_LONG=""
NETWORK_SHORT=""

ANALYSIS_PAGE="Jersey/Public_transport/Analysis"
ANALYSIS_TALK="Talk:Jersey/Public_transport/Analysis"
WIKI_ROUTES_PAGE="Jersey/Public_transport/Analysis/Jersey-Routes"

ANALYSIS_OPTIONS="--timezone=$PTNA_TIMEZONE --allow-coach --check-access --check-dates --check-way-type --check-service-type --check-name-relaxed --check-stop-position --check-sequence --check-version --check-osm-separator --check-motorway-link --max-error=20 --multiple-ref-type-entries=analyze --positive-notes --coloured-sketchline --table-show-also=from,via,to"
...
PTNA_WWW_REGION_NAME="The Bailiwick of Jersey"
PTNA_WWW_REGION_LINK="https://overpass-turbo.eu/map.html?Q=%0A%5Bout%3Ajson%5D%5Btimeout%3A25%5D%3B%0A%0A(%0A%0A%20%20relation%5Bwikidata%3D'Q785'%5D%3B%0A)%3B%0Aout%20body%3B%0A%3E%3B%0Aout%20skel%20qt%3B"
...
PTNA_WWW_ROUTES_NAME="Jersey Routes"
