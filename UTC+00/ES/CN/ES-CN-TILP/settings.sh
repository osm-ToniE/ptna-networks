...
PTNA_EXTRACT_GETIDS="espania-canary"

OVERPASS_QUERY="https://overpass-api.de/api/interpreter?data=area[wikidata=Q201689][type=multipolygon];(rel(area)[~'route'~'(bus|tram|train|subway|light_rail|trolleybus|ferry|monorail|aerialway|share_taxi|funicular)'];rel(br);rel[~'type'~'route'](r);)->.routes;(.routes;<<;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
NETWORK_LONG="Transportes Insular La Palma"
NETWORK_SHORT="TILP"

ANALYSIS_PAGE="ES:La_Palma/Transporte_público/PTNA"
ANALYSIS_TALK="Talk:ES:La_Palma/Transporte_público/PTNA"
WIKI_ROUTES_PAGE="ES:La_Palma/Transporte_público/Rutas_de_La_Palma"
...
PTNA_WWW_REGION_NAME="La Palma, Canarias"
PTNA_WWW_REGION_LINK="https://overpass-turbo.eu/map.html?Q=%0A%5Bout%3Ajson%5D%5Btimeout%3A25%5D%3B%0A%0A(%0A%0A%20%20relation%5B%22wikidata%22%3D%22Q201689%22%5D%3B%0A)%3B%0Aout%20body%3B%0A%3E%3B%0Aout%20skel%20qt%3B"
...
PTNA_WWW_ROUTES_NAME="TILP Rutas"
