...
OVERPASS_QUERY="https://overpass-api.de/api/interpreter?data=area[wikidata~'^Q739651$'][type=boundary];(rel(area)[~'route'~'(bus|tram|train|subway|light_rail|trolleybus|ferry|monorail|aerialway|share_taxi|funicular)'];rel(br);rel[~'type'~'route'](r);)->.routes;(.routes;<<;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
NETWORK_LONG=""
NETWORK_SHORT=""

ANALYSIS_PAGE= #"Chiriquí/Transporte_Público/Análisis"
ANALYSIS_TALK= #"Talk:Chiriquí/Transporte_Público/Análisis"
WIKI_ROUTES_PAGE= #"Chiriquí/Rutas_Transporte_Público"
...
PTNA_WWW_REGION_NAME="Chiriquí"
PTNA_WWW_REGION_LINK="https://overpass-turbo.eu/map.html?Q=%0A%5Bout%3Ajson%5D%5Btimeout%3A25%5D%3B%0A%0A(%0A%0A%20%20relation%5B%22wikidata%22~%22^Q739651$%22%5D%3B%0A)%3B%0Aout%20body%3B%0A%3E%3B%0Aout%20skel%20qt%3B"
