...
ANALYSIS_OPTIONS="--language=de --max-error=10 --check-access --check-name-relaxed --check-stop-position --check-sequence --check-osm-separator --check-motorway-link --multiple-ref-type-entries=analyze  --relaxed-begin-end-for=train,subway,light_rail,monorail,tram --coloured-sketchline --expect-network-long"
