...
OVERPASS_QUERY="http://overpass-api.de/api/interpreter?data=area[wikidata~'^(Q191)$'][type=boundary];(rel(area)[~'route'~'(train|subway|light_rail|monorail|funicular)'];rel(br);rel[type=%27route%27](r);)-%3E.routes;(.routes;%3C%3C;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
NETWORK_LONG="Elron"
NETWORK_SHORT=""
