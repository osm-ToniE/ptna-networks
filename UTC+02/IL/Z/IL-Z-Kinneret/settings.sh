...
OVERPASS_QUERY="https://overpass-api.de/api/interpreter?data=[timeout:900];area[wikidata='Q65073769'];(rel(area)[~'route'~'(bus|tram|train|subway|light_rail|trolleybus|ferry|monorail|aerialway|share_taxi|funicular)'];rel(br);rel[type=%27route%27](r);)-%3E.routes;(.routes;%3C%3C;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
NETWORK_LONG=""
NETWORK_SHORT=""

ANALYSIS_PAGE="Israel/Public_Transport/PTNA"
ANALYSIS_TALK="Talk:Israel/Public_Transport/PTNA"
WIKI_ROUTES_PAGE="Israel/Public_Transport/PTNA/IL-Z-Kinneret-Routes"
...
PTNA_WWW_REGION_NAME="מחוז הצפון, נפת כנרת / Kinneret Subdistrict, North District"
PTNA_WWW_REGION_LINK="https://overpass-turbo.eu/map.html?Q=%0A%5Bout%3Ajson%5D%5Btimeout%3A25%5D%3B%0A%0A(%0A%0A%20%20relation%5Bwikidata%3D'Q65073769'%5D%3B%0A)%3B%0Aout%20body%3B%0A%3E%3B%0Aout%20skel%20qt%3B"
...
PTNA_WWW_ROUTES_NAME="Kinneret, North District Routes"
