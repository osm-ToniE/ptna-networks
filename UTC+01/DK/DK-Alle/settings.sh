...
PTNA_EXTRACT_GETIDS="denmark"

OVERPASS_QUERY="https://overpass-api.de/api/interpreter?data=[timeout:1800];area[boundary=administrative][admin_level=2][name='Danmark'];(rel(area)[~'route'~'(bus|tram|train|subway|light_rail|trolleybus|ferry|monorail|aerialway|share_taxi|funicular)'];rel(br);rel[~'type'~'route'](r);)->.routes;(.routes;<<;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
#NETWORK_LONG=""
#NETWORK_SHORT=""
NETWORK_LONG=""
NETWORK_SHORT=""
