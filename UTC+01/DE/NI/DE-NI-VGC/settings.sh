...
ANALYSIS_OPTIONS="--language=de --expect-network-long --expect-network-long-for='Verkehrsgemeinschaft Cloppenburg' --expect-network-long-as='Verkehrsgemeinschaft Landkreis Cloppenburg' --check-gtfs --link-gtfs --show-gtfs --gtfs-feed=$PREFIX --max-error=10 --check-access --check-way-type --check-service-type --check-name-relaxed --check-stop-position --check-sequence --check-version --check-osm-separator --check-motorway-link --multiple-ref-type-entries=allow --positive-notes --coloured-sketchline --relaxed-begin-end-for=train,subway,light_rail,monorail,tram"
