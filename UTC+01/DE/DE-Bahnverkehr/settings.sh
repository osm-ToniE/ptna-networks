...
OVERPASS_QUERY="http://overpass-api.de/api/interpreter?data=[timeout:900];area[boundary=administrative][admin_level=2][name='Deutschland'];(rel(area)[route~'(train|light_rail|monorail)'];rel(br);rel[type='route'](r);)->.routes;(.routes;<<;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
...
PTNA_WWW_NETWORK_NAME="Deutsche Bahn und viele weitere"
