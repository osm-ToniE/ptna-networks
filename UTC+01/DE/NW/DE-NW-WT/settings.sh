...
OVERPASS_QUERY="http://overpass-api.de/api/interpreter?data=[timeout:300];area[boundary=public_transport][name~'WestfalenTarif.'];(rel(area)[route~'(bus|tram|train|subway|light_rail|trolleybus|ferry|monorail|aerialway|share_taxi|funicular)'];rel(br);<<;rel[type='route'](r);)->.routes;(.routes;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
