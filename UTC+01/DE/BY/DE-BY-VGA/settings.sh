...
OVERPASS_QUERY="http://overpass-api.de/api/interpreter?data=area[boundary=administrative][admin_level=6][name='Landkreis Eichstätt'];(rel(area)[route~'(bus|tram|train|subway|light_rail|trolleybus|ferry|monorail|aerialway|share_taxi|funicular)'];rel(br);rel[type='route'](r);)->.routes;(.routes;<<;rel(r.routes);way(r.routes);node(w);node(r.routes););out;"
NETWORK_LONG="Verkehrsgmeinschaft Altmühltal|Jägle Verkehrsbetriebe|Regionalbus Augsburg|Regionalbus Augsburg GmbH|Ingolstädter Verkehrsgesellschaft mbH|Verkehrsverbund Großraum Nürnberg|Verkehrsverbund Großraum Nürnberg GmbH|Verkehrsgemeinschaft Landkreis Kelheim|Stadtlinie Eichstätt|Sillner|Buchberger Reisen"
...
PTNA_WWW_NETWORK_NAME="Verkehrsgmeinschaft Altmühltal und weitere"
