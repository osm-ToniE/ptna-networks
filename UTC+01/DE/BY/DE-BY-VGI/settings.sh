...
OVERPASS_QUERY="https://overpass-api.de/api/interpreter?data=area[boundary=administrative][admin_level=6][name~'(Ingolstadt|Landkreis Eichstätt|Landkreis Pfaffenhofen an der Ilm|Landkreis Neuburg-Schrobenhausen)'];(rel(area)[~'route'~'(bus|tram|train|subway|light_rail|trolleybus|ferry|monorail|aerialway|share_taxi|funicular)'];rel(br);rel[~'type'~'route'](r);)->.routes;(.routes;<<;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
NETWORK_LONG="Zweckverband Verkehrsverbund Großraum Ingolstadt|Verkehrsverbund Großraum Ingolstadt|Bayerische Eisenbahngesellschaft|Regionalbus Ostbayern|Stadtlinie Eichstätt|Verkehrsgemeinschaft Landkreis Kelheim|Verkehrsverbund Großraum Nürnberg"
NETWORK_SHORT="VGI|BEG|RBO|VLK|VGN"
...
ANALYSIS_OPTIONS="--timezone=$PTNA_TIMEZONE --language=de --check-bus-stop --link-gtfs --show-gtfs --gtfs-feed=$PREFIX --max-error=10 --check-access --check-dates --check-way-type --check-service-type --check-name-relaxed --check-stop-position --check-sequence --check-version --check-osm-separator --check-motorway-link --check-roundabouts --expect-network-long --multiple-ref-type-entries=analyze --coloured-sketchline --relaxed-begin-end-for=train,subway,light_rail,monorail,tram"
...
PTNA_WWW_REGION_LINK="https://overpass-turbo.eu/map.html?Q=[out%3Ajson][timeout%3A25]%3B(relation[boundary%3Dadministrative][admin_level%3D6][name~%27(Ingolstadt|Landkreis%20Eichst%C3%A4tt|Landkreis%20Pfaffenhofen%20an%20der%20Ilm|Landkreis%20Neuburg-Schrobenhausen)%27]%3B)%3Bout%20body%3B%3E%3Bout%20skel%20qt%3B"
