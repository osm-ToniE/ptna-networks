...
OVERPASS_QUERY="https://overpass-api.de/api/interpreter?data=[timeout:600];area[wikidata=Q3139][type=boundary];(rel(area)[~'route'~'(tram|bus)'];rel(br);rel[~'type'~'route'](r);)->.routes;(.routes;<<;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
NETWORK_LONG="Envibus|Sillages|Palm Bus|Zest"
NETWORK_SHORT=""

ANALYSIS_PAGE="France:Alpes-Maritimes/Transports_en_commun/Analyse"
ANALYSIS_TALK="Talk:France:Alpes-Maritimes/Transports_en_commun/Analyse"
WIKI_ROUTES_PAGE="France:Alpes-Maritimes/Transports_en_commun/Lignes_Alpes-Maritimes"
...
PTNA_WWW_REGION_NAME="Alpes-Maritimes"
...
PTNA_WWW_ROUTES_NAME="Lignes Alpes-Maritimes"
