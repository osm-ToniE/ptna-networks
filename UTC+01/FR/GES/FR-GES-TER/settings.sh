...
PTNA_EXTRACT_SOURCE="grand-est.osm.pbf"
...
OVERPASS_QUERY="https://overpass-api.de/api/interpreter?data=[timeout:600];area[wikidata='Q18677983'][type=boundary];(rel(area)[~'route'~'(bus|tram|train|subway|light_rail|trolleybus|ferry|monorail|aerialway|share_taxi|funicular)'];rel(br);rel[~'type'~'route'](r);)->.routes;(.routes;<<;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
NETWORK_LONG="TER Grand Est"
NETWORK_SHORT=""

ANALYSIS_PAGE="France/Voies_ferrées"
ANALYSIS_TALK="Talk:France/Voies_ferrées"
WIKI_ROUTES_PAGE="TER_Grand_Est/PTNA"
...
PTNA_WWW_REGION_NAME="Grand Est"
...
PTNA_WWW_ROUTES_NAME="Lignes GES"
