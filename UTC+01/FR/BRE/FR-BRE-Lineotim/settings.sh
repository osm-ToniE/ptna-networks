...
OVERPASS_QUERY="https://overpass-api.de/api/interpreter?data=[timeout:600];area[wikidata~'^(Q584641|Q547847)$'][type=boundary];(rel(area)[~'route'~'(bus|tram|train|subway|light_rail|trolleybus|ferry|monorail|aerialway|share_taxi|funicular)'];rel(br);rel[~'type'~'route'](r);)->.routes;(.routes;<<;rel(r.routes);way(r);node(w);way(r.routes);node(w);node(r.routes););out;"
NETWORK_LONG="Linéotim"
NETWORK_SHORT=""

ANALYSIS_PAGE="User:Mga_geo/Transports_en_commun/LinéoTim/PTNA"
ANALYSIS_TALK="User_talk:Mga_geo/Transports_en_commun/LinéoTim/PTNA"
WIKI_ROUTES_PAGE="User:Mga_geo/Transports_en_commun/LinéoTim/PTNA_csv"
...
PTNA_WWW_REGION_NAME="Arrondissements de Lannion (Côtes-d'Armor) et Morlaix (Finistère)"
