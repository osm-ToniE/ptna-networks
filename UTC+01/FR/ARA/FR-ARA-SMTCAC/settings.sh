...
ANALYSIS_OPTIONS="--language=fr --positive-notes --link-gtfs --show-gtfs --gtfs-feed=$PREFIX --max-error=10 --check-platform --check-bus-stop --check-access --check-dates --check-way-type --check-service-type --check-name-relaxed --check-stop-position --check-sequence --check-version --check-osm-separator --multiple-ref-type-entries=analyze --coloured-sketchline --relaxed-begin-end-for=train,subway,light_rail,monorail,tram"
