...
#OVERPASS_QUERY="http://overpass-api.de/api/interpreter?data=area[boundary=administrative][admin_level=6][name~'(Stuttgart|Landkreis Ludwigsburg|Rems-Murr-Kreis|Landkreis Göppingen|Landkreis Esslingen|Landkreis Böblingen|Landkreis Tübingen|Landkreis Reutlingen|Ostalbkreis|Landkreis Schwäbisch Hall)'];(rel(area)[route~'(bus|tram|train|subway|light_rail|trolleybus|ferry|monorail|aerialway|share_taxi|funicular)'];rel(br);rel[type='route'](r);)->.routes;(.routes;<<;rel(r.routes);way(r.routes);node(r.routes););out;"
